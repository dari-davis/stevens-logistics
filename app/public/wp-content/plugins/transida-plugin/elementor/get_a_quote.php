...
class Get_A_Quote extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Get A Quote', 'transida' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'get_a_quote',
			[
				'label' => esc_html__( 'Get A Quote', 'transida' ),
			]
		);
		$this->add_control(
			'bg_img',
		...
		$this->add_control(
			'bg_img_v2',
				[
				  'label' => __( 'Right Side Background Image', 'transida' ),
				  'type' => Controls_Manager::MEDIA,
				  'default' => ['url' => Utils::get_placeholder_image_src(),],
				]
	    );
		...
		$this->add_control(
			'id',
			[
				'label'       => __( 'FeedBurner ID', 'transida' ),
				'type'        => Controls_Manager::TEXT,
				'label_block' => true,
				'dynamic'     => [
					'active' => true,
				],
				'placeholder' => __( 'Enter your FeedBurner ID', 'transida' ),
			]
		);
        ...
        <section class="getaquote-section style-one">
            <div class="auto-container">
                <div class="row no-gutters">
                    <div class="left-column" <?php if($settings['bg_img']['id']): ?>style="background-image: url(<?php echo esc_url(wp_get_attachment_url($settings['bg_img']['id']));?>);"<?php endif; ?>>
                        <div class="inner-container">
                            <div class="sec-title light">
                                <div class="sub-title"><?php echo wp_kses($settings['subtitle'], true);?></div>
                                <h2><?php echo wp_kses($settings['title'], true);?> </h2>
                            </div>
                            <?php echo do_shortcode($settings['quote_form_url']);?>
                        </div>
                    </div>
                    <div class="right-column" <?php if($settings['bg_img_v2']['id']): ?>style="background-image: url(<?php echo esc_url(wp_get_attachment_url($settings['bg_img_v2']['id']));?>);"<?php endif; ?>>
                        <div class="inner-container">
                            <div class="investor wow fadeInRight" data-wow-duration="1500ms">
                                ...
                                <form method="get" action="http://feedburner.google.com/fb/a/mailverify" accept-charset="utf-8">
                                    <input type="hidden" id="uri2" name="uri" value="<?php echo esc_attr($settings['id']);?>">
                                    <input type="email" placeholder="<?php esc_html_e('Your Email Address', 'transida'); ?>">
                                    <button type="submit" class="theme-btn btn-style-one"><span><i class="flaticon-up-arrow"></i><?php esc_html_e('Submit', 'transida'); ?></span></button>
                                </form>
                            </div>
                        </div>
                    </div>
