...
class Our_Industries extends Widget_Base {
	...
	public function get_name() {
		return 'transida_our_industries';
	}
	...
	public function get_title() {
		return esc_html__( 'Our Industries', 'transida' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'our_industries',
			[
				'label' => esc_html__( 'Our Industries', 'transida' ),
			]
		);
		$this->add_control(
			'bg_img',
		...
		$this->add_control(
			'transparent_title',
			[
				'label'       => __( 'Transparent Title', 'transida' ),
				'type'        => Controls_Manager::TEXT,
				'label_block' => true,
				'dynamic'     => [
					'active' => true,
				],
				'placeholder' => __( 'Enter your Transparent Title', 'transida' ),
			]
		);
		$this->add_control(
			'text_limit',
	...
	protected function render() {
		...
		if( transida_set( $settings, 'query_category' ) ) $args['project_cat'] = transida_set( $settings, 'query_category' );
        ...
        <!-- Industries Covered -->
        <section class="industries-covered" <?php if($settings['bg_img']['id']): ?>style="background-image: url(<?php echo esc_url(wp_get_attachment_url($settings['bg_img']['id']));?>);"<?php endif; ?>>
            <div class="background-text" data-parallax='{"x": 100}'><?php echo wp_kses($settings['transparent_title'], true);?></div>
                ...
                <div class="outer-container">
                    <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": true, "center": true, "margin": 0, "autoheight":true, "lazyload":true, "nav": true, "dots": true, "autoplay": true, "autoplayTimeout": 6000, "smartSpeed": 1000, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "1" }, "768" :{ "items" : "1" } , "992":{ "items" : "1" }, "1200":{ "items" : "1" }}}'>
                        ...
                        <div class="text-block">
                            <div class="inner-box">
                                <div class="image" style="background-image: url(<?php echo esc_url($post_thumbnail_url);?>);">
                                </div>
                                <div class="content">
                                    <div class="icon"><span class="<?php echo wp_kses(str_replace( "icon ",  "",  get_post_meta(get_the_id(), 'project_icon', true )), true); ?>"></span></div>
                                    <h4><?php the_title(); ?></h4>
                                    <div class="text"><?php echo wp_kses(transida_trim(get_the_content(), $settings['text_limit']), true); ?> </div>
                                    <div class="link">
                                        <a href="<?php echo esc_url(get_post_meta( get_the_id(), 'project_link', true ));?>" class="readmore-link"><i class="flaticon-up-arrow"></i><?php esc_html_e('More Details', 'transida'); ?></a>
                                    </div>
                                </div>
                            </div>
                        </div>
