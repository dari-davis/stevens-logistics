...
function get_history_categories() {
	$options = array();
...
//get Global Network categories
function get_network_categories() {
	$options = array();
	$taxonomy = 'network_cat';
	if (!empty($taxonomy)) {
		$terms = get_terms(
				array(
					'parent' => 0,
...
function transida_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('transida_redux_custom_fonts_load', 'transida_redux_custom_fonts_load');
function transida_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = transida_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/transida/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [transida_social_share_output description]
 *
 ...
 */
function transida_social_share_output( $icon, $color = false ) {
...
function transida_get_social_icons()
{
	...
	if( transida_set( $options, 'social_media' ) && is_array( transida_set( $options, 'social_media' ) ) )
	{
		$total = count( transida_set( $options, 'social_media' ) ) - 2;
		foreach( transida_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = transida_set( $social_icon, 'title');
			$class = transida_set( $social_icon, 'class');
			$link = transida_set( $social_icon, 'social_link');
			$icon = transida_set( $social_icon, 'social_icon');
...
add_action('transida_social_share_output', 'transida_social_share_output');
...
<ul class="social-links">
    ...
    <li class="facebook"><a href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank"><span class="fab fa-facebook-f"></span></a></li>
    <?php endif;?>

    <?php if($options->get( 'twitter_sharing' )):?>
    <li class="twitter"><a href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>" target="_blank"><span class="fab fa-twitter"></span></a></li>
    <?php endif;?>

    <?php if($options->get( 'linkedin_sharing' )):?>
    <li class="linkedin"><a href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fab fa-linkedin"></span></a></li>
    <?php endif;?>

    <?php if($options->get( 'pinterest_sharing' )):?>
    <li class="pinteret"><a href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fab fa-pinterest"></span></a></li>
    <?php endif;?>

    <?php if($options->get( 'reddit_sharing' )):?>
    <li class="android"><a href="http://reddit.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fab fa-reddit"></span></a></li>
    <?php endif;?>

    <?php if($options->get( 'tumblr_sharing' )):?>
    <li class="dribbble"><a href="http://www.tumblr.com/share/link?url=<?php echo esc_url(get_permalink($PostID)); ?>&name=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fab fa-tumblr"></span></a></li>
    <?php endif;?>

    <?php if($options->get( 'digg_sharing' )):?>
    <li class="rss"><a href="http://digg.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><span class="fab fa-digg"></span></a></li>
    <?php endif;?>
...
function transida_get_social_icons2()
{
