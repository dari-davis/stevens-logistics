...
class Options {
	 ...
	 * @var string
	 */
	private $opt_name = TRANSIDA_NAME . '_options';
	...
	private $page_slug = TRANSIDA_NAME . '_options';
	...
	function init() {
		...
		$this->menu_title = esc_html__( 'TRANSIDA Options', 'transida' );
		$this->page_title = esc_html__( 'TRANSIDA Options', 'transida' );
	...
	function args() {
		...
		$args = array(
			...
			'global_variable'      => 'transida_options',
	...
	function helpTabs() {
		...
		$tabs = array(
			array(
				'id'      => 'redux-help-tab-1',
				'title'   => esc_html__( 'Theme Information 1', 'transida' ),
				'content' => esc_html__( '<p>This is the tab content, HTML is allowed.</p>', 'transida' ),
			),
			array(
				'id'      => 'redux-help-tab-2',
				'title'   => esc_html__( 'Theme Information 2', 'transida' ),
				'content' => esc_html__( '<p>This is the tab content, HTML is allowed.</p>', 'transida' ),
			),
		);
		...
		$content = '<p>' . esc_html__( 'This is the sidebar content, HTML is allowed.', 'transida' ) . '</p>';
	...
	function sections() {
		...
		$sections_path = apply_filters( 'transida_redux_sections', $sections_path );
		...
		foreach ( $sections_path as $key => $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				$options = apply_filters( "transida_redux_sections_{$key}", $options );
