...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => transida_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => transida_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => transida_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => transida_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => transida_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => transida_meta( 'banner_elementor_template', $page_id ),
				'banner'            => transida_set( transida_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => transida_meta( 'banner_banner_title', $page_id ),
				'text'             => transida_meta( 'banner_banner_text', $page_id ),
				'enable_banner'     => transida_meta( 'banner_page_banner', $page_id ),
			];
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : transida_the_title( $template ),
				'text'         => $options->get( $text ) ? $options->get( $text ) : transida_the_title( $template ),
				'banner'        => transida_set( $bg, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$return = [
			...
			'banner_elementor'  => transida_meta( 'banner_elementor_template', $page_id ),
			'banner'            => transida_set( transida_meta( 'banner_page_background', $page_id ), 'url' ),
			'title'             => ($title) ? $title : esc_html__('', 'transida'),
			'text'             => ($text) ? $text : esc_html__('', 'transida'),
			'enable_banner'     => transida_meta( 'banner_page_banner', $page_id ),
