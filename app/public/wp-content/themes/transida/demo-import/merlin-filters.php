 ...
 */
function transida_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'transida_unset_default_widgets_args' );
 ...
 */
function transida_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'transida_child_functions_php', 10, 2 );
 ...
 */
function transida_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'transida'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'transida_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'transida' ),
			'preview_url'              => 'http://commonsupport.net/newwp/transida/',
		),
	);
}

add_filter( 'merlin_import_files', 'transida_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function transida_after_import_setup() {
	...
	$blog_page_id  = get_page_by_title( 'Large Image' );
...
add_action( 'merlin_after_all_import', 'transida_after_import_setup' );

/**
 * Transida 2 mime types.
 * 
 * @param  [type] $mimes [description]
 * @return [type]        [description]
 */
function transida_mime_types($mimes) {
 $mimes['svg'] = 'image/svg+xml';
 return $mimes;
}
add_filter('upload_mimes', 'transida_mime_types');